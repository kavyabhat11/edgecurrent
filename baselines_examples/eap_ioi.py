...
def parse_args():
    ...
    p.add_argument("--sparsities", default="93,94,95,96,96.5,96.6,97,97.5,98,98.5,99")
    p.add_argument("--out-dir", default="results/ioi-eap-paper")
    return p.parse_args()
...
@torch.no_grad()
def full_model_logits(model, tokens, batch_size):
    outs = []

    for i in tqdm(range(0, tokens.shape[0], batch_size), desc="Full model eval"):
...
def main():
    ...
    full_logits = full_model_logits(model, eval_toks, args.eval_batch_size)
