"""EAP baseline for BLiMP subject-verb agreement (and similar minimal-pair categories).

Forks the structure of eap_ioi_paper.py, swapping the IOI data loader for a
BLiMP-style loader. Each example is a (good, bad) sentence pair differing at
exactly one mid-sentence token. We treat:
  clean = sentence_good
  corrupted = sentence_bad
  target = the differing token in sentence_good
  distractor = the differing token in sentence_bad

Logit-difference metric and KL evaluation are unchanged from IOI's wrapper.
"""
...
def parse_args():
    ...
    p.add_argument("--dataset-path", default="data/datasets/blimp_regular_plural_subject_verb_agreement_1/")
    p.add_argument("--train-split", default="train")
    p.add_argument("--eval-split", default="test")
    p.add_argument("--train-examples", type=int, default=200)
    p.add_argument("--eval-examples", type=int, default=600)
    p.add_argument("--batch-size", type=int, default=1)
    p.add_argument("--eval-batch-size", type=int, default=1)
    p.add_argument("--device", default=("cuda" if torch.cuda.is_available() else "cpu"))
    p.add_argument("--sparsities", default="93,94,95,96,97,98,99")
    p.add_argument("--out-dir", default="results/blimp-eap-paper-kl")
    return p.parse_args()
...
def get_blimp_data(ds, tokenizer, max_examples):
    if max_examples is not None and max_examples < len(ds):
        ds = ds.select(range(max_examples))

    clean_texts = list(ds["sentence_good"])
    corr_texts = list(ds["sentence_bad"])
    targets = list(ds["target_word"])
    distractors = list(ds["distractor_word"])
    prefixes = list(ds["prefix"])
    ...
    for i in range(len(ds)):
        # The differing word is at position prefix_token_count under shared-prefix.
        # We want the model to predict the differing token given the prefix.
        prefix_ids = tokenizer.encode(prefixes[i] if prefixes[i] else " ")
        # If tokenizer prepended BOS, strip it for length count.
        if prefix_ids and prefix_ids[0] == bos:
            prefix_len_real = len(prefix_ids) - 1
        else:
            prefix_len_real = len(prefix_ids)
        # The "predict-the-differing-token" position is the last index of the prefix
        # in the *full* tokenized sentence. Since clean_texts has BOS prepended (if any)
        # by the tokenizer we used, account for that.
        full_ids = tokens[i].tolist()
        prepended_bos = (full_ids[0] == bos) if bos is not None else False
        pred_pos = (1 if prepended_bos else 0) + prefix_len_real - 1
        # Sanity: pred_pos should be < attention-masked length
        last_real_pos = int(attn[i].sum().item()) - 1
        if pred_pos < 0 or pred_pos > last_real_pos:
            # Fallback: place at last_real_pos - 1
            pred_pos = max(0, last_real_pos - 1)

        target_tok = first_real(tokenizer.encode(" " + targets[i]))
        distractor_tok = first_real(tokenizer.encode(" " + distractors[i]))

        pred_indices.append(pred_pos)
        correct_tokens.append(target_tok)
...
def kl_model_circuit(circuit_logits, full_logits, indices):
    ...
    return F.kl_div(circuit_log_probs, full_log_probs, log_target=True, reduction="batchmean")
...
def circuit_logits_for_edges(model, clean_tokens, corr_tokens, pred_indices, keep_edges, batch_size):
    ...
    upstream_diff = torch.zeros(
        ...
        device=model.cfg.device, dtype=model.cfg.dtype, requires_grad=False,
    )
...
def main():
    ...
    train_toks, train_corr_toks, train_idx, train_correct, train_distractor = get_blimp_data(
    ...
    eval_toks, eval_corr_toks, eval_idx, eval_correct, eval_distractor = get_blimp_data(
    ...
    csv_path = os.path.join(args.out_dir, f"eap_blimp_{args.eval_split}_metrics.csv")
