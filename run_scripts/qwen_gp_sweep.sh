#!/bin/bash
# Edge Pruning sweep on Qwen2-0.5B for GP (Gendered Pronoun).
...
WANDB_MODE=disabled python src/prune/fqwen_gp.py \
    --report_to wandb \
    --do_train \
    --do_eval \
    ...
    --output_dir ./data/runs/qwen-gp-${TAG}-es${EDGE_SPARSITY}-ns${NODE_SPARSITY}-t${TOTAL}/ \
