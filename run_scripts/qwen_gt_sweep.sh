#!/bin/bash
# Edge Pruning sweep on Qwen2-0.5B for GT (Greater Than).
...
WANDB_MODE=disabled python src/prune/fqwen_gt.py \
    --report_to wandb \
    --do_train \
    --do_eval \
    ...
    --output_dir ./data/runs/qwen-gt-${TAG}-es${EDGE_SPARSITY}-ns${NODE_SPARSITY}-t${TOTAL}/ \
