...
def parse_args():
    ...
    parser.add_argument("--data-path", "-d", default="./data/datasets/blimp_regular_plural_subject_verb_agreement_1/")
...
def main():
    ...
    for i in tqdm(range(len(data))):
        sentence = data[i]['sentence_good']
        corr_sentence = data[i]['sentence_bad']
        target = data[i]['target_word']
        distractor = data[i]['distractor_word']
    ...
    for i in bar:
        ...
        for j in range(input_ids.shape[0]):
            # BLiMP: prefix is the shared substring up to (but not including) the differing token.
            sentence_prefix = prefixes[i + j] if prefixes[i + j] else " "
