...
from l0_fqwen import deterministic_z_from_log_alpha, sample_z_from_log_alpha
...
class FQwen2RotaryEmbedding(nn.Module):
    ...
    def sin_cached(self):
        logger.warning_once(
            ...
            "the forward method of RoPE from now on instead. It is not used in the `Qwen2Attention` class"
        )
...
class FQwen2LinearScalingRotaryEmbedding(FQwen2RotaryEmbedding):
...
class FQwen2DynamicNTKScalingRotaryEmbedding(FQwen2RotaryEmbedding):
...
class FQwen2MLP(nn.Module):
    ...
    def forward(self, x):
        if getattr(self.config, "pretraining_tp", 1) > 1:
            slice = self.intermediate_size // getattr(self.config, "pretraining_tp", 1)
            gate_proj_slices = self.gate_proj.weight.split(slice, dim=0)
            up_proj_slices = self.up_proj.weight.split(slice, dim=0)
            down_proj_slices = self.down_proj.weight.split(slice, dim=1)

            gate_proj = torch.cat(
                [F.linear(x, gate_proj_slices[i]) for i in range(getattr(self.config, "pretraining_tp", 1))], dim=-1
            )
            up_proj = torch.cat([F.linear(x, up_proj_slices[i]) for i in range(getattr(self.config, "pretraining_tp", 1))], dim=-1)

            intermediate_states = (self.act_fn(gate_proj) * up_proj).split(slice, dim=2)
            down_proj = [
                F.linear(intermediate_states[i], down_proj_slices[i]) for i in range(getattr(self.config, "pretraining_tp", 1))
            ]
...
class FQwen2Attention(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        # Qwen2 always has bias=True on q/k/v_proj (independent of any config flag) and bias=False on o_proj.
        # Llama parameterizes this via config.attention_bias; Qwen2Config has no such field, so hardcode.
    ...
    def _apply_headwise_linear(self, x, weight, num_heads, bias=None):
        ...
        # bias (optional) is (num_q_or_kv_heads * head_dim,) -- needed for Qwen2 q/k/v_proj
        ...
        if bias is not None:
            # bias [num_heads * head_dim] -> [1, num_heads, 1, head_dim] for broadcasting
            projected = projected + bias.view(num_heads, self.head_dim).unsqueeze(0).unsqueeze(2)
        return projected
    ...
    def forward(
        ...
        **kwargs,
        ...
        if getattr(self.config, "pretraining_tp", 1) > 1:
            assert self.num_heads % getattr(self.config, "pretraining_tp", 1) == 0, "Number of heads must be divisible by pretraining_tp"
            assert self.num_key_value_heads % getattr(self.config, "pretraining_tp", 1) == 0, "Number of key value heads must be divisible by pretraining_tp"
            ...
            q_hidden_states = q_hidden_states.split(self.num_heads // getattr(self.config, "pretraining_tp", 1), dim=-1)
            query_states = [
                self._apply_headwise_linear(
                    q_hidden_states[i],
                    query_slices[i],
                    self.num_heads // getattr(self.config, "pretraining_tp", 1)
                )
                for i in range(getattr(self.config, "pretraining_tp", 1))
            ] 
            ...
            k_hidden_states = k_hidden_states.split(self.num_key_value_heads // getattr(self.config, "pretraining_tp", 1), dim=-1)
            key_states = [
                self._apply_headwise_linear(
                    k_hidden_states[i],
                    key_slices[i],
            ...
            v_hidden_states = v_hidden_states.split(self.num_key_value_heads // getattr(self.config, "pretraining_tp", 1), dim=-1)
            value_states = [
                self._apply_headwise_linear(
                    v_hidden_states[i],
                    value_slices[i],
            ...
            query_states = self._apply_headwise_linear(q_hidden_states, self.q_proj.weight, self.num_heads, bias=self.q_proj.bias)
            key_states = self._apply_headwise_linear(k_hidden_states, self.k_proj.weight, self.num_key_value_heads, bias=self.k_proj.bias)
            value_states = self._apply_headwise_linear(v_hidden_states, self.v_proj.weight, self.num_key_value_heads, bias=self.v_proj.bias)
        ...
        if getattr(self.config, "pretraining_tp", 1) > 1:
            attn_output = attn_output.split(self.num_heads // getattr(self.config, "pretraining_tp", 1), dim=1)
            o_proj_slices = self.o_proj.weight.split(self.num_heads * self.head_dim // getattr(self.config, "pretraining_tp", 1), dim=1)
            attn_output = sum([
...
class FQwen2FlashAttention2(FQwen2Attention):
...
class FQwen2SdpaAttention(FQwen2Attention):
    # Adapted from FQwen2Attention.forward
    def forward(
        self,
        q_hidden_states: torch.Tensor,
        k_hidden_states: torch.Tensor,
...
if __name__ == '__main__':
    model = FQwen2ForCausalLM.from_pretrained("Qwen/Qwen2-0.5B", torch_dtype=torch.bfloat16, with_embedding_nodes=True).to("cuda")
