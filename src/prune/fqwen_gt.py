...
from modeling_fqwen import FQwen2LMHeadModel

require_version("datasets>=1.8.0", "To fix: pip install -r examples/pytorch/text-classification/requirements.txt")
logger = logging.getLogger(__name__)

class FQwen2InfoTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        with torch.no_grad():
            ...
            ref_logits = self.ref_model(input_ids=input_ids, **inputs).logits
            ref_logits = torch.gather(
                ref_logits, 
                1, 
                indices.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, ref_logits.shape[-1])
            ).squeeze()
            ref_logits = torch.gather(ref_logits, 1, self.digits.unsqueeze(0).repeat(ref_logits.shape[0], 1))
        ...
        kl_loss = nn.functional.kl_div(logits, ref_logits, reduction="batchmean", log_target=True)
...
def main():
    ...
    model = FQwen2LMHeadModel.from_pretrained(
    ...
    ref_model = FQwen2LMHeadModel.from_pretrained(
        "Qwen/Qwen2-0.5B",
        with_embedding_nodes=data_args.with_embedding_nodes,
    ).to("cuda")
