...
class DataTrainingArguments:
    dataset_path: Optional[str] = field(
        default="./data/datasets/blimp_regular_plural_subject_verb_agreement_1/",
...
class DataCollatorBLiMP:
    """Collator for BLiMP minimal pairs (extension experiment).

    Each example must have:
      sentence_good, sentence_bad : full sentences (paired)
      prefix                      : shared prefix up to (but not including) the differing token

    Loss is computed at the single token position where the two sentences diverge:
    the model's prediction at end-of-prefix should be the grammatical word in
    sentence_good vs the ungrammatical alternative in sentence_bad. This matches
    IOI's logit-diff structure; only the diff position is mid-sentence rather
    than at the final word.
    """
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            corr_text = example["sentence_bad"]
            prefix = example["prefix"] if example["prefix"] else " "

            len_prefix = self.tokenizer(prefix, return_tensors="pt").input_ids.shape[1]
            ...
            corr_input_ids = self.tokenizer(
                corr_text, return_tensors="pt", max_length=self.max_length,
                padding="max_length", truncation=True,
            ).input_ids[0]
