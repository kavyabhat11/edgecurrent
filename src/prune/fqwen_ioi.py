...
class FQwen2InfoTrainer(Seq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        with torch.no_grad():
            ...
            logits_ref = self.ref_model(input_ids=input_ids, **inputs).logits
        ...
        for i in range(logits.shape[0]):
            ...
            logits_ref_i = nn.functional.log_softmax(logits_ref[i, start_idxes[i]:end_idxes[i]], dim=-1)
            
            kl_loss_component = nn.functional.kl_div(logits_i, logits_ref_i, reduction='batchmean', log_target=True)
